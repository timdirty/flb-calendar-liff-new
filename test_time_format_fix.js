...
async function testTimeFormatFix() {
    ...
    try {
        ...
        const testCases = [
            "一1930-2030 到府",  // 需要修復：添加星期和時間之間的空格
            "二1430-1530 客製化", // 需要修復：添加星期和時間之間的空格
            "三1630-1730 到府",  // 需要修復：添加星期和時間之間的空格
            "四1930-2030",       // 需要修復：添加星期和時間之間的空格
            "五1930-2100 到府",  // 需要修復：添加星期和時間之間的空格
            "一 1930-2030 到府", // 已經是正確格式，不需要修復
            "二 1430-1530"       // 已經是正確格式，不需要修復
        ];
        ...
        for (const testCase of testCases) {
            ...
            const result = await page.evaluate((timeString) => {
                ...
                if (fixedTimeString && fixedTimeString.includes(' ')) {
                    ...
                    fixedTimeString = fixedTimeString.replace(/^([一二三四五六日])(\d{4}-\d{4})\s+(.+)$/, '$1 $2 $3');
                } else if (fixedTimeString && !fixedTimeString.includes(' ')) {
                    // 匹配格式：星期 + 時間（完全沒有空格）
                    // 例如："一1930-2030" -> "一 1930-2030"
                    fixedTimeString = fixedTimeString.replace(/^([一二三四五六日])(\d{4}-\d{4})$/, '$1 $2');
                }
                return {
                    original: timeString,
