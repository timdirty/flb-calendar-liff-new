...
async function testAttendanceProgressFix() {
    ...
    try {
        ...
        page.on('console', msg => {
            const text = msg.text();
            if (text.includes('簽到') || text.includes('通知') || text.includes('進度') || text.includes('標記') || text.includes('學生') || text.includes('出席') || text.includes('缺席')) {
                ...
                if (presentButtons.length > 0) {
                    ...
                    const progressInfo = await page.evaluate(() => {
                        ...
                        cards.forEach(card => {
                            ...
                            // 檢查是否有當次簽到狀態
                            if (window.studentAttendanceStatus && window.studentAttendanceStatus[name]) {
                                currentMarked++;
                            }
                            ...
                            if (window.studentAttendanceStatus && window.studentAttendanceStatus[name]) {
                                markedStudents++;
                            } else if (statusText && statusText !== '未簽到' && statusText !== '待簽到') {
                                markedStudents++;
                            }
                        });
